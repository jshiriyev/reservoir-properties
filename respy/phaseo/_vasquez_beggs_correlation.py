...
class VasquezBeggs:
	...
	@staticmethod
	def gass(p:np.ndarray,sgsg:float,gAPI:float,temp:float,psep:float=None,Tsep:float=None):
		"""
	...
	def fvf(Rs:np.ndarray,sgsg:float,gAPI:float,temp:float,psep:float=None,Tsep:float=None):
	...
	def comp(p:np.ndarray,Rsb:float,sgsg:float,gAPI:float,temp:float,psep:float=None,Tsep:float=None):
